use bit_vec::BitVec;
use bitter::{
    peer::{PeerHandler, DownloadParams, run_peer_handler}, metainfo::{Metainfo, PeerId}, bencoding::bdecode, accounting::Accounting, protocol::{TcpConn, Handshake, Packet}
};
use tempdir::TempDir;
use std::{fs, sync::Arc, collections::HashSet, env};
use tokio::{self, io::{DuplexStream, duplex}};
...
async fn basic() {
    ...
    conn.write(&Packet::Handshake(Handshake::Bittorrent(&metainfo.info.hash, &my_peer_id))).await.unwrap();
    assert_eq!(conn.read_handshake().await.unwrap(), Handshake::Bittorrent(&metainfo.info.hash, &PeerId::default()));
        ...
        match packet {
            ...
            Packet::Request { index, begin, length } => {
                conn.write(&Packet::Piece { 
                    index, 
                    begin, 
                    data: &pieces[index as usize][begin as usize..(begin + length) as usize]
                }).await.unwrap();
